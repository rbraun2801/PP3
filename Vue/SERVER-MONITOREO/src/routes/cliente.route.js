...
router.put('/cliente/:id', async (req, res) => {
  ...
  db.query(`update cliente
    set razon_social='${cmod.razon_social}', telefono='${cmod.telefono}', direccion_ip='${cmod.direccion_ip}', descripcion_equipo='${cmod.descripcion_equipo}',sistema_operativo='${cmod.sistema_operativo}'
     where id_cliente=${id}`)
...
router.get('/cliente/:dni', async (req,res) => {
  ...
  const db = await conexion.obtener_conexion()
  const row = await db.query(`select * from cliente where razon_social = ${razon_social} `)
  res.json(row[0])
})
